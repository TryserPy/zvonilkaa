...
class WebSocketConnection extends EventEmitter {
  constructor(socket, maxPayload) {
    ...
    socket.on('error', (err) => {
      // EventEmitter без слушателя 'error' роняет процесс — а одно битое
      // соединение не должно уносить с собой весь сервер
  ...
  _write(opcode, payload) {
    ...
    if (!sock || sock.destroyed || sock.writableEnded || this.readyState === this.CLOSED) return;
